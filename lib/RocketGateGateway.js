...
RocketGateGateway.prototype.sendRequest = function (params, guid) {
  ...
  return post(uri, {
    ...
    .then(function (response) {
      var json = response.gatewayResponse;

      if (json.reasonCode[0] !== '0') {
        throw new GatewayError(reasonCodes[json.reasonCode[0].toString()] || 'Unknown error from the gateway', json);
      }
      return json;
    });
};
...
RocketGateGateway.prototype.submitTransaction = function submitTransaction (order, creditCard, prospect, other) {
  ...
  return this.sendRequest(params)
    .then(function (res) {
      output = {
        transactionId: res.guidNo[0],
        authCode: res.authNo[0],
        _original: res
      };
      return self.confirmTransaction(res.guidNo[0]);
    })
    .then(function (res) {
      return output;
    });
...
RocketGateGateway.prototype.createCustomerProfile = function createCustomerProfile (cc, billing, shipping, other) {
  ...
  return this.authorizeTransaction({amount: 1}, cc, prospect, other)
    .then(function (res) {
      return {
        _original: res._original,
        profileId: res._original.cardHash[0]
      };
    });
};
